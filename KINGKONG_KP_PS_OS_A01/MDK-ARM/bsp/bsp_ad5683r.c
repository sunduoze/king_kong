...
/**
  ...
  * @retval None
  */ 
void ad5683r_set_volt(float volt_val)
{
	#define AD5683_VREF 1.25f// MD 不知为何 凸(艹皿艹 )
	static float vol_tmp;
	if(volt_val > 0.0f)
	{
		vol_tmp = volt_val / AD5683_VREF;
		if(volt_val < 1.25f)
		{
			AD5683R_Write_Value_L(vol_tmp * 65536);
		}
		else if(volt_val < 2.5f)
		{
			AD5683R_Write_Value_H((uint32_t)((vol_tmp - 1) * 65536));
		}
		else
		{
